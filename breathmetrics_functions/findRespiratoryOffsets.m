...
for bi = 1:length(exhaleOnsets)
    if isnan(inhalePauseOnsets(bi))
        inhaleOffsets(1, bi) = exhaleOnsets(bi);
    else
        inhaleOffsets(1, bi) = inhalePauseOnsets(bi);
    end
end
...
for bi = 1:length(exhaleOnsets) - 1
    if isnan(exhalePauseOnsets(bi))
        exhaleOffsets(1, bi) = inhaleOnsets(bi + 1);
    else
        exhaleOffsets(1, bi) = exhalePauseOnsets(bi);
    end
end
...
if isempty(putativeExhaleOffset) || putativeExhaleOffset < lowerLim || ...
    ...
    exhaleOffsets(1,end) = exhaleOnsets(1,end) + putativeExhaleOffset;
end
